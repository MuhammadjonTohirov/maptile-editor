...
run_ogr() {
  docker run --rm --network "$NETWORK" -v "$PBF:/data/extract.osm.pbf:ro" "$GDAL_IMAGE" \
    ogr2ogr -f PostgreSQL "$PG_OGR" /data/extract.osm.pbf "$@"
}
...
run_ogr multipolygons -nln multipolygons -where "building IS NOT NULL" "${COMMON_LCO[@]}" "${SPAT_ARGS[@]}"

log "Staging roads (lines)"
run_ogr lines -nln lines -where "highway IS NOT NULL" "${COMMON_LCO[@]}" "${SPAT_ARGS[@]}"

log "Staging street furniture (points)"
run_ogr points -nln points -where "highway IN ('traffic_signals','street_lamp')" "${COMMON_LCO[@]}" "${SPAT_ARGS[@]}"
